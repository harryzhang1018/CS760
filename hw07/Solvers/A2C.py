...
class A2C(AbstractSolver):
    ...
    def train(self, states, actions, rewards, next_state, done):
        ...
        # and actions
        discounted_sum = 0 if done else self.actor_critic.value(torch.tensor(next_state, dtype=torch.float32))
        for t in reversed(range(len(rewards))):
            discounted_sum = discounted_sum * self.options.gamma + rewards[t]
